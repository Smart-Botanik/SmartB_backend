import type { PlantPlacementStage, PrismaClient } from "@prisma/client";
...
type PlacementFixture = {
  slug: string;
  stage: PlantPlacementStage;
  locationName: string;
  seatLabel?: string;
};

const PLACEMENT_FIXTURES: PlacementFixture[] = [
...
async function upsertPlacementPlant(
  prisma: PrismaClient,
  userId: string,
  ...
  const baseData = {
    ...
    plannedLocationId:
      fixture.stage === "planned" || fixture.stage === "ready_to_plant" ? locationId : null,
...
export async function seedDevPlantPlacement(prisma: PrismaClient) {
  ...
  for (const fixture of PLACEMENT_FIXTURES) {
    ...
    results.push(
      await upsertPlacementPlant(prisma, user.id, fixture, growbox.id, seatId),
    );
  }
