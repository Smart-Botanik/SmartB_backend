...
async function expectRejects(action, expectedMessage) {
    try {
        ...
        assert(error instanceof Error && error.message.includes(expectedMessage), `Expected error containing "${expectedMessage}", got "${error instanceof Error ? error.message : String(error)}"`);
        return;
    }
...
async function main() {
    ...
    try {
        ...
        assert(profile.kind === client_1.RegistryProfileKind.event_write, "Expected event_write profile");
        assert(JSON.stringify(profile.fields.map((field) => field.fieldId)) ===
            JSON.stringify(WATERING_FIELD_IDS), "Expected watering.event.v1 field order from seed");
        ...
        assert(updated.fields.find((field) => field.fieldId === "plant.watering.solution.ph")?.required ===
            true, "Expected solution pH to be required");
        ...
        assert(chartProfile.kind === client_1.RegistryProfileKind.timeseries_read, "Expected timeseries_read");
        assert(JSON.stringify(chartProfile.fields.map((field) => field.fieldId)) ===
            JSON.stringify(WATERING_CHART_FIELD_IDS), "Expected watering.chart.v1 numeric slice without nutrients");
        ...
        assert(snapshotProfile.kind === client_1.RegistryProfileKind.snapshot_build, "Expected snapshot_build");
        assert(JSON.stringify(snapshotProfile.fields.map((field) => field.fieldId)) ===
            JSON.stringify(WATERING_FIELD_IDS), "Expected current.snapshot.v1 to include all includeInCurrent watering fields");
        ...
        await expectRejects(() => service.setProfileFields({
            profileKey: WATERING_PROFILE_KEY,
        ...
        assert(JSON.stringify(preview.payload) ===
            JSON.stringify({
                watering: {
        ...
        assert(missingRequired.errors.some((error) => error.code === "required"), "Expected required error for missing solution pH");
        const invalidNumber = await service.buildPreview({
            profileKey: WATERING_PROFILE_KEY,
            valuesJson: { "plant.watering.solution.ph": 6.25 },
        });
        assert(invalidNumber.errors.some((error) => error.code === "precision" || error.code === "step"), "Expected precision or step error for invalid pH");
        ...
        assert(JSON.stringify(locationProfile.fields.map((field) => field.fieldId)) ===
            JSON.stringify(LOCATION_EQUIPMENT_FIELD_IDS), "Expected location.indoor.equipment.v1 field order from seed");
        ...
        assert(locationPreview.errors.length === 0, `Expected no location preview errors, got ${JSON.stringify(locationPreview.errors)}`);
        assert(JSON.stringify(locationPreview.payload) ===
            JSON.stringify({
                equipment: {
                    enclosure: {
        ...
        assert(JSON.stringify(diaryProfile.fields.map((field) => field.fieldId)) ===
            JSON.stringify(DIARY_SETUP_CONFIG_FIELD_IDS), "Expected diary.setup.config.v1 field order from seed");
        ...
        assert(diaryPreview.errors.length === 0, `Expected no diary preview errors, got ${JSON.stringify(diaryPreview.errors)}`);
        assert(JSON.stringify(diaryPreview.payload) ===
            JSON.stringify({
                diary: {
                    wateringType: "drip",
                    roomType: "indoor",
        ...
        assert(deprecatedRow.status === client_1.RegistryFieldSpecStatus.deprecated, "Expected deprecate mutation to set deprecated status");
        ...
        assert(idempotent.status === client_1.RegistryFieldSpecStatus.deprecated, "Expected idempotent deprecate to keep deprecated status");
        ...
        assert(phUsage.profileKeys.includes(WATERING_PROFILE_KEY), "Expected watering.event.v1 to reference solution pH");
        await expectRejects(() => service.getFieldSpecUsage("plant.registry.nonexistent.e2e"), "Registry field spec not found");
        const diaryProfileForUser = await service.getProfileByKey(DIARY_SETUP_CONFIG_PROFILE_KEY, contracts_1.Role.USER);
        assert(diaryProfileForUser, "Expected USER to read diary.setup.config.v1 profile");
        assert(diaryProfileForUser.fields.map((field) => field.fieldId).join(",") ===
            DIARY_SETUP_CONFIG_FIELD_IDS.join(","), "Expected diary.setup.config.v1 field order for USER");
        const diaryFieldSpecsForUser = await service.listFieldSpecs({
            entity: "Diary",
            role: contracts_1.Role.USER,
        });
        assert(diaryFieldSpecsForUser.length === DIARY_SETUP_CONFIG_FIELD_IDS.length, "Expected USER Diary field spec slice");
        const adminOnlyProfile = await service.getProfileByKey(WATERING_CHART_PROFILE_KEY, contracts_1.Role.USER);
...
//# sourceMappingURL=test-registry-profile-e2e.js.map