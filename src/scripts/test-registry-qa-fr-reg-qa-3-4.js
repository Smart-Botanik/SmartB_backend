...
const DIARY_FIELD_IDS = ["diary.setup.watering_type", "diary.setup.room_type"];
const WATERING_RUNTIME_INPUTS = {
...
async function loadProfileBuildRows(service, profileKey) {
    ...
    return profile.fields.map(profileField => {
        ...
        const field = {
            ...
            formatJson: spec.formatJson ?? null,
            constraintsJson: spec.constraintsJson ?? null,
        };
...
function buildFromRuntimeInputs(rows, inputs) {
    return (0, contracts_1.buildNormalizedValuesJson)(rows.map(row => ({
        field: row.field,
        pattern: row.pattern,
...
async function runQa3(service) {
    ...
    assert(JSON.stringify(profile.fields.map(field => field.fieldId)) ===
        JSON.stringify(WATERING_FIELD_IDS), "FR-REG-QA-3: profile field picker must match seed order");
    ...
    assert(normalized.errors.length === 0, `FR-REG-QA-3: normalizer must pass for runtime inputs, got ${JSON.stringify(normalized.errors)}`);
    const preview = await service.buildPreview({
        profileKey: WATERING_PROFILE_KEY,
        valuesJson: normalized.valuesJson,
    });
    assert(preview.errors.length === 0, `FR-REG-QA-3: buildPreview must succeed, got ${JSON.stringify(preview.errors)}`);
    assert(JSON.stringify(preview.payload) === JSON.stringify(EXPECTED_WATERING_PAYLOAD), "FR-REG-QA-3: payload must match watering canonical tree");
    const ecWithoutProfile = (0, contracts_1.buildNormalizedValuesJson)([
        {
            field: {
    ...
    assert(ecWithoutProfile.errors.some(error => error.code === "conversion_requires_profile"), "FR-REG-QA-3: EC→PPM without conversionProfile must fail in normalizer before backend");
    ...
    assert(advancedPreview.errors.length === 0, "FR-REG-QA-3: Advanced JSON canonical valuesJson must pass buildPreview");
}
async function runQa4(service) {
    ...
    assert(JSON.stringify(profile.fields.map(field => field.fieldId)) ===
        JSON.stringify(DIARY_FIELD_IDS), "FR-REG-QA-4: diary profile fields must match seed");
    ...
    assert(wateringTypeRow?.field.formatJson?.mode === "select", "FR-REG-QA-4: watering_type must use select format for UI");
    assert(roomTypeRow?.field.formatJson?.mode === "select", "FR-REG-QA-4: room_type must use select format for UI");
    const normalized = buildFromRuntimeInputs(rows, DIARY_RUNTIME_INPUTS);
    assert(normalized.errors.length === 0, `FR-REG-QA-4: diary normalizer must pass, got ${JSON.stringify(normalized.errors)}`);
    const preview = await service.buildPreview({
        profileKey: DIARY_PROFILE_KEY,
        valuesJson: normalized.valuesJson,
    });
    assert(preview.errors.length === 0, `FR-REG-QA-4: diary buildPreview must succeed, got ${JSON.stringify(preview.errors)}`);
    assert(JSON.stringify(preview.payload) === JSON.stringify(EXPECTED_DIARY_PAYLOAD), "FR-REG-QA-4: diary payload must use camelCase canonical paths");
    assert(CREATE_DIARY_EVENT_PROFILE_KEY === DIARY_PROFILE_KEY, "FR-REG-QA-4: Create Diary Event pilot must reuse diary.setup.config.v1 profileKey");
}
async function main() {
...
//# sourceMappingURL=test-registry-qa-fr-reg-qa-3-4.js.map