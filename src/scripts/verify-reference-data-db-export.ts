/**
 * Verifies pre-cutover monolith Brand/Product counts against reference_data_db.
...
async function main() {
  ...
  try {
    const [sourceBrands, targetBrands, sourceProducts, targetProducts] =
      await Promise.all([
    ...
    if (sourceBrands !== targetBrands) {
      throw new Error(
        `Brand count mismatch: source=${sourceBrands} target=${targetBrands}`,
      );
    }
    if (sourceProducts !== targetProducts) {
      throw new Error(
        `Product count mismatch: source=${sourceProducts} target=${targetProducts}`,
      );
    }

    let sourceLinkCount = 0;
    try {
      const linkRows = await source.$queryRaw<Array<{ count: bigint }>>`
    ...
    const targetLinkRows = await target.$queryRaw<Array<{ count: bigint }>>`
    ...
    if (sourceLinkCount !== targetLinkCount) {
      throw new Error(
        `Taxonomy link count mismatch: source=${sourceLinkCount} target=${targetLinkCount}`,
      );
    }

    const sourceBrandIds = await source.$queryRaw<Array<{ id: string }>>`
      SELECT id FROM "Brand"
    `;
    const targetBrandIds = new Set(
      (await target.brand.findMany({ select: { id: true } })).map(b => b.id),
    );
    ...
    console.log(
      `[db:verify-reference-data-export] OK — ${targetBrands} brands, ${targetProducts} products, ${targetLinkCount} taxonomy links`,
    );
  } finally {
